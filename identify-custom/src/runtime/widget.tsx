...
const Widget = (props: AllWidgetProps<any>) => {
  ...
  const mapView = viewManager.getJimuMapViewById(
    viewManager.getAllJimuMapViewIds()[0]
  );
  ...
  useEffect(() => {
    if (props.mutableStateProps?.newSearch) {
      getAppStore().dispatch(
        appActions.widgetStatePropChange(`widget_${sidebarId}`, "expand", true)
      );
      //Timeout to wait for map to start moving.
      setTimeout(() => {
        reactiveUtils.when(
          () =>
            jimuMapView.view.stationary &&
            !jimuMapView.view.updating &&
          ...
          () => {
            projection.load().then(() => {
              ...
              const screenPoint = jimuMapView.view.toScreen(
                props.mutableStateProps.newSearch
              );
  ...
  const createEventListener = () => {
    const eventListener = mapView.view.on("click", (event) => {
      const featureContainer = document.getElementById("identifyDiv");
      featureContainer.innerHTML = "";
      setEvent(event);
      setContainer(featureContainer);
  ...
  return (
    <div style={{ height: "inherit", overflow: "auto" }}>
      ...
      <div className="d-flex justify-content-center mt-2">
        <Label>
          {on
            ? "Disable Map Click Search Results"
            : "Enable Map Click Search Results"}
          <Switch
            className="ml-2"
      ...
      <div id="identifyDiv">
        &nbsp;&nbsp;Click the map to identify features.
      </div>
      {event ? (
        <Waiting
          mapView={mapView}
        ...
        <div>{on ? <></> : <p>Enable switch to identify features.</p>}</div>
      )}
    </div>
  );
};
